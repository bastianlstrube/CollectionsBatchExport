...
bl_info = {
    ...
    "author": "Bastian Leonhardt Strube, brockmann",
    "version": (0, 2, 0),
    "blender": (4, 0, 3),
    "location": "File > Import-Export",
    "description": "Batch Export Object to FBX",
    "warning": "",
    "wiki_url": "",
    "tracker_url": "",
...
class Batch_FBX_Export(bpy.types.Operator, ExportHelper):
    ...
    # context group
    object_selection_setting: EnumProperty(
            name="Objects",
    ...
    def execute(self, context):                
        ...
        # Collections
        if self.object_selection_setting == 'Collections':
            for collection in bpy.data.collections:
                ...
                for obj in collection.objects:
                    obj_export_list.append(obj)

                # add to dict
                obj_export_dict[collection.name] = obj_export_list

        # Selected Objects
        if self.object_selection_setting == 'Selection':
            obj_export_list = viewport_selection
            for obj in obj_export_list:
                obj_export_dict[obj.name] = [obj]

        # All Scene Objects
        if self.object_selection_setting == 'All Objects':
        ...
        for name, items in obj_export_dict.items():
            for item in items:
